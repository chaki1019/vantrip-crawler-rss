...
const getPlatpark = async (): Promise<Array<Platpark>> => {
  let ret: Array<Platpark> = [];
  const browser = await playwright['chromium'].launch();
  const BASE_URL = `https://sapa.c-nexco.co.jp/guide/platpark`

  try {
    const list: Array<Platpark> = []
    const context = await browser.newContext();
    const page = await context.newPage();
    await page.goto(BASE_URL)
    const td = await page.locator('//*[@class="hidden-sp"]/*[@class="mod-table"]//td').all()
    for (let i = 0; i < td.length; i += 3) {
      const name = (await td[i].textContent() as string).replace(/MAP/, '').trim()
      const url = new URL(await (await td[i].locator('a')).getAttribute('href') as string, BASE_URL).toString()
      const open_time = (await td[i + 1].textContent() as string).trim()
      const description = (await td[i + 2].textContent() as string).trim()

      list.push({
        name, url, open_time, description
      })
    }
    ...
    for (let i = 0; i < list.length;) {
      ...
      try {
        await page.goto(platpark.url, {timeout: 60000})
        const scripts = await page.locator('//script[not(@src)]').all()
        for (let i = 0; i < scripts.length; i++) {
          const s = await scripts[i].textContent()
          const matched = s?.match(/new google.maps.LatLng\((\d.+?),\s*(\d.+?)\)/)
          if (matched) {
            platpark.lat = parseFloat(matched[1])
            platpark.lng = parseFloat(matched[2])
            break
          }
        }
        const imageExist = (await page.locator('//*[@class="guide-area-map"]/img').count()) > 0
        if (imageExist) {
          const image = await page.locator('//*[@class="guide-area-map"]/img').first().getAttribute('src') as string
          const imageUrl = new URL(image, platpark.url)
          platpark.image = imageUrl.toString()
        }
        
        logger.debug(i, platpark)
        i++
        retryCount = 0
      } catch (error) {
        if (retryCount >= 3) {
